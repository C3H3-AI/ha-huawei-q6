  ...
  `;async connectedCallback(){super.connectedCallback(),this.addEventListener("refresh-stats",()=>this._loadStats()),this.addEventListener("detail",e=>this._openDetail(e.detail.repo)),this.addEventListener("favorite",()=>this._loadStats()),this._keydownHandler=e=>{if("Escape"===e.key&&this._showDetail)return e.preventDefault(),void this._closeDetail();if((e.ctrlKey||e.metaKey)&&"k"===e.key){e.preventDefault();const t=this.renderRoot?.querySelector(".search input");return void(t&&(t.focus(),t.select()))}if(!this._showDetail&&"INPUT"!==document.activeElement?.tagName&&"SELECT"!==document.activeElement?.tagName&&"TEXTAREA"!==document.activeElement?.tagName){const t=["browse","updates","management"],o=parseInt(e.key);o>=1&&o<=t.length&&(e.preventDefault(),this.switchView(t[o-1]))}},window.addEventListener("keydown",this._keydownHandler)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("keydown",this._keydownHandler)}async _loadStats(){try{this._error="",this.stats=await ce.getStats();const e=await ce.getFavorites(),t=Array.isArray(e)?e:e.favorites||[];this._favoriteCount=t.length}catch(e){console.error("Stats error:",e),this.stats={},this._error=`API: ${e.message}`}}switchView(e){this.currentView!==e&&(this._viewTransition=!0,this.currentView=e,this.dispatchEvent(new CustomEvent("view-changed",{detail:{view:e},bubbles:!0,composed:!0})),setTimeout(()=>{this._viewTransition=!1},150))}_openDetail(e){this._detailRepo=e,this._showDetail=!0,this._readmeHtml=null,this._readmeLoading=!0,this._showVersionSelector=!1,this._releases=[],this._releasesLoading=!1,this._changelogData=null,this._changelogLoading=!0,this._loadReadme(e),this._loadChangelog(e)}async _loadReadme(e){if(e?.full_name){try{const t=await ce.getReadme(e.full_name);this._readmeHtml=t?Rt.sanitize(t):null}catch(e){this._readmeHtml=null}this._readmeLoading=!1}else this._readmeLoading=!1}async _loadChangelog(e){if(e?.full_name){try{const t=await ce.getChangelog(e.full_name);this._changelogData=t?.body?t:null}catch(e){this._changelogData=null}this._changelogLoading=!1}else this._changelogLoading=!1}_closeDetail(){this._showDetail=!1,this._detailRepo=null,this._readmeHtml=null,this._readmeLoading=!1,this._detailExpanded=!1,this._showVersionSelector=!1,this._releases=[],this._releasesLoading=!1}_toggleDetailExpand(){this._detailExpanded=!this._detailExpanded}async _toggleVersionSelector(){if(this._showVersionSelector=!this._showVersionSelector,this._showVersionSelector&&0===this._releases.length){this._releasesLoading=!0;try{const e=this._detailRepo?.id||this._detailRepo?.full_name;if(e){const t=await ce.getRepoReleases(e);this._releases=Array.isArray(t)?t:t.releases||[]}}catch(e){console.error("Failed to load releases:",e),this._releases=[]}this._releasesLoading=!1}}async _installVersion(e){const t=this._detailRepo?.id||this._detailRepo?.full_name;if(t&&e){this._installingVersion=!0;try{await ce.installVersion(t,e),It(`${ue("installComplete")}: ${e}`,"success"),this.dispatchEvent(new CustomEvent("refresh-stats",{bubbles:!0,composed:!0})),this._closeDetail()}catch(e){It(`${ue("installFailed")}: ${e.message}`,"error")}this._installingVersion=!1}}_getCategoryLabel(e){return{integration:ue("catIntegration"),plugin:ue("catPlugin"),theme:ue("catTheme"),appdaemon:ue("catAppDaemon"),netdaemon:ue("catNetDaemon"),python_script:ue("catPython"),template:ue("catTemplate")}[e]||e}async _modalAction(e){const t=this._detailRepo;if(t)try{if("install"===e)await ce.install(t.id||t.full_name,t.category),It(ue("installComplete"),"success");else if("update"===e)await ce.update([t.id||t.full_name]),It(ue("updateStarted"),"success");else if("uninstall"===e){const{ConfirmDialog:e}=await Promise.resolve().then(function(){return Mt});if(!await e.show(this,{message:`${ue("confirmRemove")} ${t.full_name||t.name}?`,confirmText:ue("remove"),danger:!0}))return;await ce.remove(t.id||t.full_name),It(ue("removed"),"success")}else if("github"===e){const e=t.html_url||`https://github.com/${t.full_name}`;return void window.open(e,"_blank")}this.dispatchEvent(new CustomEvent("refresh-stats",{bubbles:!0,composed:!0})),this._closeDetail()}catch(e){It(`${ue("updateFailed")}: ${e.message}`,"error")}}_checkTabsScrollable(){const e=this.renderRoot?.querySelector(".tabs-wrapper"),t=e?.querySelector(".tabs");e&&t&&e.classList.toggle("scrollable",t.scrollWidth>t.clientWidth)}async updated(e){super.updated(e),e.has("narrow")&&requestAnimationFrame(()=>this._checkTabsScrollable())}firstUpdated(){requestAnimationFrame(()=>this._checkTabsScrollable())}render(){const e=[{view:"browse",label:ue("tabBrowse"),icon:"🔍"},{view:"updates",label:ue("tabUpdates"),icon:"🔄",count:this.stats.available_updates},{view:"management",label:ue("tabManagement"),icon:"⚙️"}],t=this._detailRepo,o=t?.installed||!1,a=o&&(t?.has_update||t?.installed_version&&t?.latest_version&&t.installed_version!==t.latest_version),r=t?me(t.category):"#03a9f4";return H`
      <div class="store">
        ...
        <div class="content ${this._viewTransition?"transitioning":""}">
          ${"browse"===this.currentView?H`<browse-view .hass=${this.hass}></browse-view>`:""}
    ...
    `}}let Et=null;const Dt=[];let Nt=!1;function It(e,t="info"){const o=Et||document.querySelector("hacs-vision-panel"),a=o?.shadowRoot?.querySelector("#toast-container");a?(Dt.push({msg:e,type:t}),Nt||Ot(a)):console.warn("Toast container not found:",e)}function Ot(e){if(0===Dt.length)return void(Nt=!1);Nt=!0;const{msg:t,type:o}=Dt.shift(),a=document.createElement("div");a.className="toast",o&&a.classList.add(o),a.textContent=t,e.appendChild(a),requestAnimationFrame(()=>{a.classList.add("show")}),setTimeout(()=>{a.classList.remove("show"),setTimeout(()=>{a.remove(),Ot(e)},350)},3e3)}class Lt extends se{static properties={repo:{type:Object},_isFavorite:{type:Boolean,state:!0},_installing:{type:Boolean},_favorites:{type:Array,state:!0}};constructor(){super(),this.repo={},this._isFavorite=!1,this._installing=!1,this._favorites=[]}async connectedCallback(){super.connectedCallback(),await this._loadFavorites()}async _loadFavorites(){try{const e=await ce.getFavorites();this._favorites=Array.isArray(e)?e:e.favorites||[]}catch(e){this._favorites=[]}this._updateFavoriteState()}_updateFavoriteState(){this.repo&&(this._isFavorite=this._favorites.includes(this.repo.id||this.repo.full_name))}willUpdate(e){e.has("repo")&&this.repo&&this._updateFavoriteState()}static styles=i`
  ...
  `;_getInitials(e){if(!e)return"?";const t=e.split("/");return(t[t.length-1]||e).charAt(0).toUpperCase()}_getCategoryLabel(e){return{integration:ue("catIntegration"),plugin:ue("catPlugin"),theme:ue("catTheme"),appdaemon:ue("catAppDaemon"),netdaemon:ue("catNetDaemon"),python_script:ue("catPython"),template:ue("catTemplate")}[e]||e}_handleAction(e,t){e.stopPropagation(),this.dispatchEvent(new CustomEvent(t,{detail:{repo:this.repo},bubbles:!0,composed:!0}))}async _handleFavorite(e){e.stopPropagation();const t=this.repo.id||this.repo.full_name,o=[...this._favorites],a=o.indexOf(t);a>=0?(o.splice(a,1),this._isFavorite=!1):(o.push(t),this._isFavorite=!0);try{await ce.setFavorites(o),this._favorites=o}catch(e){this._isFavorite=!this._isFavorite}this.dispatchEvent(new CustomEvent("favorite",{detail:{repo:this.repo,isFavorite:this._isFavorite},bubbles:!0,composed:!0}))}_handleCardClick(){this.dispatchEvent(new CustomEvent("detail",{detail:{repo:this.repo},bubbles:!0,composed:!0}))}render(){const e=this.repo,t=e.manifest_name||e.repository_manifest?.name||e.full_name||e.name||"unknown",o=e.description||"",a=e.category||"integration",r=e.stars||e.stargazers_count||0,i=e.downloads||0,s=e.installed||!1,n=s&&(e.has_update||e.installed_version&&e.latest_version&&e.installed_version!==e.latest_version),l=me(a);return H`
      <div class="card" @click=${this._handleCardClick}>
        <div class="img-container">
          ...
          ${e.is_custom?H`<span class="badge custom-badge">${ue("customBadge")}</span>`:""}
          ${s?H`<span class="installed-badge">✅ ${ue("installed")}</span>`:""}
        ...
        <div class="actions">
          ...
          `:H`
            <button class="action-btn primary ${this._installing?"installing":""}"
              ...
              ${this._installing?H`⏳ ${ue("installing")}`:H`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg><span class="label">${ue("install")}</span>`}
            </button>
          `}
        </div>
      </div>
    `}}customElements.define("repo-card",Lt);const Ut=i`
  /* ===== Loading ===== */
  .loading {
...
`;class Pt extends se{static properties={_message:{type:String,state:!0},_confirmText:{type:String,state:!0},_cancelText:{type:String,state:!0},_danger:{type:Boolean,state:!0},_visible:{type:Boolean,state:!0}};constructor(){super(),this._message="",this._confirmText=ue("confirm"),this._cancelText=ue("cancel"),this._danger=!1,this._visible=!1,this._resolve=null}static styles=i`
  ...
  `;static show(e,{message:t,confirmText:o,cancelText:a,danger:r}={}){let i=e.shadowRoot?.querySelector("confirm-dialog");return i||(i=document.createElement("confirm-dialog"),e.shadowRoot.appendChild(i)),new Promise(e=>{i._resolve=e,i._message=t||"Are you sure?",i._confirmText=o||ue("confirm"),i._cancelText=a||ue("cancel"),i._danger=!!r,i._visible=!0})}_onConfirm(){this._visible=!1,this._resolve&&this._resolve(!0)}_onCancel(){this._visible=!1,this._resolve&&this._resolve(!1)}_onOverlayClick(e){e.target===e.currentTarget&&this._onCancel()}render(){return this._visible?H`
  ...
  `;async connectedCallback(){super.connectedCallback(),await this._loadFavorites(),await this._load(),this.addEventListener("install",e=>this._handleInstall(e.detail.repo)),this.addEventListener("update",e=>this._handleUpdate(e.detail.repo)),this.addEventListener("uninstall",e=>this._handleUninstall(e.detail.repo)),this.addEventListener("detail",e=>this._handleDetail(e.detail.repo)),this.addEventListener("readme",e=>this._handleDetail(e.detail.repo)),this.addEventListener("favorite",()=>this._syncFavoriteCount())}async _loadFavorites(){try{const e=await ce.getFavorites();this._favorites=Array.isArray(e)?e:e.favorites||[]}catch(e){this._favorites=[]}}_syncFavoriteCount(){this.statusCounts={...this.statusCounts,favorites:this._favorites.length},this.requestUpdate()}_persistState(){!function(e){try{localStorage.setItem(Ft,JSON.stringify(e))}catch{}}({search:this.search,category:this.category,statusFilter:this.statusFilter,sort:this.sort,sortDir:this.sortDir,page:this.page,groupBy:this.groupBy,pageSize:this.pageSize})}async _handleInstall(e){const t=e.id||e.full_name;this._installingIds={...this._installingIds,[t]:!0};try{await ce.install(t,e.category),It(`${ue("installComplete")}: ${e.full_name||e.name}`,"success"),this.dispatchEvent(new CustomEvent("refresh-stats",{bubbles:!0,composed:!0})),this._load()}catch(e){It(`${ue("installFailed")}: ${e.message}`,"error")}delete this._installingIds[t],this._installingIds={...this._installingIds}}async _handleUpdate(e){try{await ce.update([e.id||e.full_name]),this.dispatchEvent(new CustomEvent("refresh-stats",{bubbles:!0,composed:!0})),this._load()}catch(e){console.error("Update failed",e)}}async _handleUninstall(e){if(await Pt.show(this,{message:`${ue("confirmRemove")} ${e.full_name||e.name}?`,confirmText:ue("remove"),danger:!0}))try{await ce.remove(e.id||e.full_name),this.dispatchEvent(new CustomEvent("refresh-stats",{bubbles:!0,composed:!0})),this._load()}catch(e){console.error("Uninstall failed",e)}}_handleDetail(e){this.dispatchEvent(new CustomEvent("detail",{detail:{repo:e},bubbles:!0,composed:!0}))}async _load(){this.loading=!0;try{const e=await ce.listRepositories({search:this.search,category:this.category,sort:this.sort,sortDir:this.sortDir,page:this.page,limit:this.limit,status:this.statusFilter});this.repos=e.repositories||[],this.total=e.total||0,this.categoryCounts=e.category_counts||{},this.statusCounts=e.status_counts||{},this.statusCounts={...this.statusCounts,favorites:this._favorites.length}}catch(e){console.error("Browse load error",e),this.repos=[],this.total=0}this.loading=!1}_onSearch(e){this._searchText=e.target.value,clearTimeout(this._searchTimer),this._searchTimer=setTimeout(()=>{this.search=this._searchText,this.page=1,this._persistState(),this._load()},300)}_clearSearch(){this._searchText="",this.search="",this.page=1,this._persistState(),this._load()}_onStatusFilter(e){this.statusFilter=e,this.page=1,this._persistState(),this._load()}_onTypeFilter(e){this.category=e,this.page=1,this._persistState(),this._load()}_onSortColumn(e){this.sort===e?this.sortDir="desc"===this.sortDir?"asc":"desc":(this.sort=e,this.sortDir="name"===e?"asc":"desc"),this.page=1,this._persistState(),this._load()}_onGroupChange(e){this.groupBy=e.target.value,this._persistState()}_onViewModeChange(e){this.viewMode=e;try{localStorage.setItem(Bt,e)}catch{}}_toggleGroup(e){this._collapsedGroups={...this._collapsedGroups,[e]:!this._collapsedGroups[e]}}_goPage(e){this.page=e,this._persistState(),this._load()}_onPageSizeChange(e){this.pageSize=parseInt(e.target.value,10),this.limit=this.pageSize,this.page=1,this._persistState(),this._load()}_refresh(){this.page=1,this._persistState(),this._load()}async _addRepo(){if(this._newRepoUrl.trim()){this._addRepoInstalling=!0;try{const e=await ce.addCustomRepo(this._newRepoUrl.trim(),this._newRepoCategory);e.success?(It(`${ue("installComplete")}: ${this._newRepoUrl.trim()}`,"success"),this._newRepoUrl="",this._showAddRepo=!1,this._load(),this.dispatchEvent(new CustomEvent("refresh-stats",{bubbles:!0,composed:!0}))):It(`${ue("addFailed")}: ${e.error||"unknown"}`,"error")}catch(e){It(`${ue("addFailed")}: ${e.message}`,"error")}this._addRepoInstalling=!1}}_getRepoStatus(e){return e.id||e.full_name,e.pending_restart?"pending_restart":e.installed&&(e.has_update||e.installed_version&&e.latest_version&&e.installed_version!==e.latest_version)?"pending-upgrade":e.installed?"installed":e.new||"new"===e.status?"new":"default"}_getStatusBadge(e){const t={installed:{label:ue("statusInstalled"),cls:"installed"},"pending-upgrade":{label:ue("statusPendingUpgrade"),cls:"pending-upgrade"},"pending-restart":{label:ue("statusPendingRestart"),cls:"pending-restart"},new:{label:ue("statusNew"),cls:"new"},default:{label:ue("statusDefault"),cls:"default"}}[e]||{label:e,cls:"default"};return H`<span class="status-badge ${t.cls}">${t.label}</span>`}_getStatusLabel(e){return{installed:ue("statusInstalled"),not_installed:ue("statusDefault"),update_available:ue("statusPendingUpgrade"),favorites:ue("statusFavorites"),new:ue("statusNew"),custom:ue("statusCustom"),pending_restart:ue("statusPendingRestart")}[e]||e}_applyFilters(e){return"favorites"===this.statusFilter?e.filter(e=>this._favorites.includes(e.id||e.full_name)):e}_groupRepos(e){if("none"===this.groupBy)return null;const t={};for(const o of e){let e;e="status"===this.groupBy?this._getRepoStatus(o):"type"===this.groupBy&&o.category||"other",t[e]||(t[e]=[]),t[e].push(o)}const o=["pending-restart","pending-upgrade","installed","new","default"],a=["integration","plugin","theme","appdaemon","netdaemon","python_script","template","other"],r=Object.keys(t);return"status"===this.groupBy?r.sort((e,t)=>o.indexOf(e)-o.indexOf(t)):"type"===this.groupBy&&r.sort((e,t)=>a.indexOf(e)-a.indexOf(t)),r.map(e=>({key:e,label:"status"===this.groupBy?this._getStatusLabel(e):this._getCategoryLabel(e),repos:t[e]}))}_getCategoryLabel(e){return{integration:ue("catIntegration"),plugin:ue("catPlugin"),theme:ue("catTheme"),appdaemon:ue("catAppDaemon"),netdaemon:ue("catNetDaemon"),python_script:ue("catPython"),template:ue("catTemplate"),other:e}[e]||e}_formatDate(e){if(!e)return"";try{const t=new Date(e),o=new Date-t,a=Math.floor(o/864e5);return 0===a?ue("today")||"今天":1===a?ue("yesterday")||"昨天":a<30?`${a}d`:a<365?`${Math.floor(a/30)}mo`:`${Math.floor(a/365)}y`}catch{return""}}_renderRepoList(e){return"list"===this.viewMode?H`<div class="list-view">${this._renderListTable(e)}</div>`:H`<div class="grid">${e.map(e=>H`<repo-card .repo=${e} ._installing=${!!this._installingIds?.[e.id||e.full_name]}></repo-card>`)}</div>`}_renderListTable(e){const t=e=>this.sort!==e?"":"desc"===this.sortDir?" ▼":" ▲",o=e=>"sortable "+(this.sort===e?"active-sort":"");return H`
    ...
    `}_renderListRow(e){const t=me(e.category||"integration"),o=e.manifest_name||e.repository_manifest?.name||e.full_name||e.name||"?",a=e.stars||e.stargazers_count||0,r=e.downloads||0,i=this._getRepoStatus(e),s=e.installed||!1,n="pending-upgrade"===i,l=e.id||e.full_name,c=!!this._installingIds?.[l];return H`
      <tr @click=${()=>this._handleDetail(e)}>
        <td class="col-icon"><div class="icon-cell" style="background:${t}">${o.charAt(0).toUpperCase()}</div></td>
        <td class="name-cell">${o}<br><span class="desc-cell">${e.description||""}</span></td>
        <td class="num-cell col-downloads">${r?r.toLocaleString():"-"}</td>
        <td class="num-cell col-stars">⭐ ${"number"==typeof a?a.toLocaleString():a}</td>
        ...
        <td class="status-cell col-status">${this._getStatusBadge(i)}</td>
        <td class="actions-cell">
          ${s?H`
            ${n?H`<button class="action-sm primary" @click=${t=>{t.stopPropagation(),this._handleUpdate(e)}}>${ue("update")}</button>`:""}
          `:H`
            <button class="action-sm primary ${c?"installing":""}" @click=${t=>{t.stopPropagation(),this._handleInstall(e)}} ?disabled=${c}>${c?"⏳":ue("install")}</button>
          `}
        </td>
      </tr>
    `}render(){const e=Math.ceil(this.total/this.limit),t=this._applyFilters(this.repos),o=this._groupRepos(t);return H`
      ...
      <div class="controls">
        ...
        <div class="controls-right">
          <div class="view-toggle">
            <button class="view-toggle-btn ${"card"===this.viewMode?"active":""}" @click=${()=>this._onViewModeChange("card")} title="${ue("viewCard")}">📊</button>
            <button class="view-toggle-btn ${"list"===this.viewMode?"active":""}" @click=${()=>this._onViewModeChange("list")} title="${ue("viewList")}">📋</button>
          </div>
      ...
      `:H`
        ...
        `:""}
      `}
    `}}customElements.define("browse-view",Ht);class jt extends se{static properties={updates:{type:Array},loading:{type:Boolean},updating:{type:Boolean},search:{type:String},_installingIds:{type:Object,state:!0},_changelogs:{type:Object,state:!0},_searchText:{type:String,state:!0},_selectedIds:{type:Object,state:!0}};constructor(){super(),this.updates=[],this.loading=!1,this.updating=!1,this.search="",this._searchTimer=null,this._installingIds={},this._changelogs={},this._searchText="",this._selectedIds={}}static styles=[Ut,i`
      :host { display: block; touch-action: manipulation; }
    ...
    `];async connectedCallback(){super.connectedCallback(),await this._load()}async _load(){this.loading=!0;try{const e=await ce.getUpdates();this.updates=Array.isArray(e)?e:e.updates||[],this._loadChangelogs()}catch(e){console.error("Failed to load updates",e),this.updates=[]}this.loading=!1}async _loadChangelogs(){const e=this.updates.filter(e=>e.full_name);if(0===e.length)return;const t=await Promise.allSettled(e.map(e=>ce.getChangelog(e.full_name).then(t=>({fullName:e.full_name,data:t})))),o={};for(const e of t)"fulfilled"===e.status&&e.value.data?.body&&(o[e.value.fullName]=e.value.data);Object.keys(o).length>0&&(this._changelogs={...this._changelogs,...o})}async _updateSelected(){const e=Object.keys(this._selectedIds).filter(e=>this._selectedIds[e]);if(0===e.length)return;if(await Pt.show(this,{message:`${ue("confirmUpdateAll")} ${e.length}?`,confirmText:ue("confirmUpdate"),danger:!1})){this.updating=!0;try{await ce.update(e),It(`${ue("allUpdatesStarted")} (${e.length})`,"success"),this._selectedIds={},this._load(),this.dispatchEvent(new CustomEvent("refresh-stats",{bubbles:!0,composed:!0}))}catch(e){It(`${ue("updateFailed")}: ${e.message}`,"error")}this.updating=!1}}_toggleSelect(e){this._selectedIds={...this._selectedIds,[e]:!this._selectedIds[e]}}_toggleSelectAll(){const e=this._getFiltered();if(this._isAllSelected())this._selectedIds={};else{const t={};for(const o of e)t[o.id||o.full_name]=!0;this._selectedIds=t}}_isAllSelected(){const e=this._getFiltered();return 0!==e.length&&e.every(e=>this._selectedIds[e.id||e.full_name])}_selectedCount(){return Object.keys(this._selectedIds).filter(e=>this._selectedIds[e]).length}async _updateOne(e){const t=e.id||e.full_name;this._installingIds={...this._installingIds,[t]:!0};try{await ce.update([t]);const o=e.latest_version;let a=0;const r=async()=>{if(a++>30)return this._installingIds={...this._installingIds},delete this._installingIds[t],void It(`${ue("installFailed")}: timeout`,"error");try{const a=await ce.getRepoStatus(t);if(a?.installed_version===o||a?.installed&&!a?.has_update)return this._installingIds={...this._installingIds},delete this._installingIds[t],It(`${ue("updateComplete")}: ${e.full_name||e.name}`,"success"),this._load(),void this.dispatchEvent(new CustomEvent("refresh-stats",{bubbles:!0,composed:!0}))}catch(e){}setTimeout(r,2e3)};setTimeout(r,2e3)}catch(e){this._installingIds={...this._installingIds},delete this._installingIds[t],It(`${ue("updateFailed")}: ${e.message}`,"error")}}_getFiltered(){if(!this.search)return this.updates;const e=this.search.toLowerCase();return this.updates.filter(t=>(t.full_name||t.name||"").toLowerCase().includes(e))}_clearSearch(){this._searchText="",this.search="",this._searchTimer&&(clearTimeout(this._searchTimer),this._searchTimer=null)}_openDetail(e){this.dispatchEvent(new CustomEvent("detail",{detail:{repo:e},bubbles:!0,composed:!0}))}render(){const e=this._getFiltered();return H`
      <div class="controls">
        <div class="search">
          <svg class="search-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/>
      ...
      `:H`
        <div class="info-bar">
          ...
          <button class="btn" @click=${this._load}>${ue("refresh")}</button>
        </div>
        ...
        ${this.updates.length>1?H`
          <div class="update-all-bar">
            <button class="update-all-btn" @click=${this._updateSelected} ?disabled=${this.updating||0===this._selectedCount()}>
              ⬆ ${this.updating?ue("updatingProgress"):`${ue("updateAll")} (${this._selectedCount()||0})`}
            </button>
          </div>
        `:""}

        <div class="grid">
          ${e.map(e=>{const t=e.id||e.full_name,o=!!this._installingIds?.[t],a=this._changelogs?.[e.full_name],r=!!this._selectedIds[t];return H`
            <div class="card" @click=${t=>{t.target.closest(".btn")||t.target.closest("a")||t.target.closest(".checkbox")||this._openDetail(e)}}>
              ...
              <button class="btn primary ${o?"installing":""}"
                ...
                ${o?H`⏳ ${ue("updatingProgress")}`:H`⬆ ${ue("updateNow")}`}
              </button>
            </div>
          `})}
        </div>
  ...
  `;async connectedCallback(){super.connectedCallback(),await this._load()}async _load(){this.loading=!0;try{const e=await ce.getConfig()||{};this.archivedRepos=e.archived_repositories||[],this.ignoredRepos=e.ignored_repositories||[],this.renamedEntries=Object.entries(e.renamed_repositories||{});const t=await ce.getCustomRepos();this.customRepos=Array.isArray(t)?t:t.custom_repositories||[]}catch(e){console.error("Config load error",e)}this.loading=!1}async _removeArchivedRepo(e){if(await Pt.show(this,{message:`${ue("confirmRemoveArchived")} ${e}?`,confirmText:ue("removeArchived"),danger:!0}))try{await ce.removeArchivedRepo(e),this._load()}catch(e){It(`${ue("removeRepoFailed")}: ${e.message}`,"error")}}async _removeRenamedRepo(e){if(await Pt.show(this,{message:`${ue("confirmRemoveRenamed")} ${e}?`,confirmText:ue("removeRenamed"),danger:!0}))try{await ce.removeRenamedRepo(e),this._load()}catch(e){It(`${ue("removeRepoFailed")}: ${e.message}`,"error")}}async _replaceRenamedOneClick(e,t){if(await Pt.show(this,{message:`${ue("confirmReplaceRenamed")}: ${e} → ${t}${ue("replaceRenamedWarning")}?`,confirmText:ue("replace"),danger:!0})){this._renamedRefreshing=!0;try{await ce.replaceRenamedRepo(e,t),It(`${ue("replace")}: ${e} → ${t}`,"success"),await ce.refresh(),this._load(),this.dispatchEvent(new CustomEvent("refresh-stats",{bubbles:!0,composed:!0}))}catch(e){It(`${ue("updateFailed")}: ${e.message}`,"error")}this._renamedRefreshing=!1}}async _checkDependencies(){this._depLoading=!0;try{const e=await ce.checkDependencies();this._depResults=e,e.all_ok?It(ue("depOk"),"success"):It(`${ue("depMissing")} (${e.issues_count})`,"error")}catch(e){this._depResults=null,It(`${ue("checkFailed")}: ${e.message}`,"error")}this._depLoading=!1}_depMissingCount(){return this._depResults?.dependencies?this._depResults.dependencies.filter(e=>e.has_issues).length:0}async _export(){this.exporting=!0;try{const e=await ce.exportBackup(),t=new Blob([JSON.stringify(e,null,2)],{type:"application/json"}),o=URL.createObjectURL(t),a=document.createElement("a");a.href=o,a.download=`hacs-vision-backup-${(new Date).toISOString().slice(0,10)}.json`,a.click(),URL.revokeObjectURL(o),It(ue("exportSuccess"),"success")}catch(e){It(`${ue("exportFailed")}: ${e.message}`,"error")}this.exporting=!1}async _import(){const e=document.createElement("input");e.type="file",e.accept=".json",e.onchange=async()=>{const t=e.files[0];if(t){this.importing=!0;try{const e=await t.text(),o=JSON.parse(e);await ce.importBackup(o),It(ue("importSuccess"),"success"),this.dispatchEvent(new CustomEvent("refresh-stats",{bubbles:!0,composed:!0}))}catch(e){It(`${ue("importFailed")}: ${e.message}`,"error")}this.importing=!1}},e.click()}_toggleAddCustom(){this._showAddCustom=!this._showAddCustom,this._showAddCustom||(this._customRepoUrl="",this._customRepoCategory="integration")}_parseRepoUrl(e){const t=(e=e.trim()).match(/github\.com\/([^/]+\/[^/\s?#]+)/i);return t?t[1].replace(/\.git$/,""):/^[a-zA-Z0-9_-]+\/[a-zA-Z0-9_.-]+$/.test(e)?e:null}async _addCustomRepo(){const e=this._parseRepoUrl(this._customRepoUrl);if(!e)return void It(ue("invalidRepoUrl"),"error");const t=this.customRepos.some(t=>(t.full_name||t.repository)===e);if(t)It(`⚠️ ${e} ${ue("alreadyExists")}`,"error");else{this._addingCustom=!0;try{const t=await ce.addCustomRepo(e,this._customRepoCategory);t.success?(It(`${ue("addSuccess")}: ${e}`,"success"),this._customRepoUrl="",this._load(),this.dispatchEvent(new CustomEvent("refresh-stats",{bubbles:!0,composed:!0}))):It(`${ue("addFailed")}: ${t.error}`,"error")}catch(e){It(`${ue("addFailed")}: ${e.message}`,"error")}this._addingCustom=!1}}async _removeCustomRepo(e,t){if(await Pt.show(this,{message:`${ue("confirmRemoveRepo")} ${e}?`,confirmText:ue("remove"),danger:!0}))try{await ce.removeCustomRepo(e),this._load(),this.dispatchEvent(new CustomEvent("refresh-stats",{bubbles:!0,composed:!0}))}catch(e){It(`${ue("removeRepoFailed")}: ${e.message}`,"error")}}_toggleSection(e){this._collapsed={...this._collapsed,[e]:!this._collapsed[e]}}_setViewMode(e){this._viewMode=e}_openCardDetail(e){this.dispatchEvent(new CustomEvent("detail",{detail:{repo:e},bubbles:!0,composed:!0}))}_getInitials(e){if(!e)return"?";const t=e.split("/");return(t[t.length-1]||e).charAt(0).toUpperCase()}_getCategoryColor(e){return{integration:"#1565c0",plugin:"#7b1fa2",theme:"#2e7d32",python_script:"#f9a825",template:"#6a1b9a",appdaemon:"#e65100",netdaemon:"#00838f",dashboard:"#f57f17"}[e]||"#78909c"}_getCategoryLabel(e){return{integration:ue("catIntegration"),plugin:ue("catPlugin"),theme:ue("catTheme"),appdaemon:ue("catAppDaemon"),netdaemon:ue("catNetDaemon"),python_script:ue("catPython"),template:ue("catTemplate"),dashboard:ue("catDashboard")}[e]||e}_renderCard(e,t){const o=e.full_name||e.repository,a=e.manifest_name||e.name||o,r=t.find(([e,t])=>t===o),i=!!r,s=i?r[0]:null,n=e.installed_version||"",l=e.latest_version||"",c=e.has_update,d=e.stargazers_count||0,p=e.description||"",h=e.installed||!1,u=this._getCategoryColor(e.category);return H`
      <div class="repo-card" @click=${()=>this._openCardDetail(e)}>
        <div class="repo-card-img" style="background:linear-gradient(135deg, ${u}44 0%, ${u}22 100%);">
          ...
          ${i?H`<span class="repo-card-renamed">🔄 ${s}</span>`:""}
          ${h?H`<span class="repo-card-installed">✅ ${ue("installed")}</span>`:""}
          <div class="repo-card-actions-img">
            <button class="btn-icon" @click=${t=>{t.stopPropagation(),this._removeCustomRepo(o,e.category)}} title="${ue("remove")}">
              ✕
            </button>
          </div>
        </div>
        <div class="repo-card-body">
          ...
          <div class="meta">
            <span class="stars">
              ⭐ ${d>0?"number"==typeof d?d.toLocaleString():d:0}
            </span>
            ${h?H`
              <span style="font-size:10px;color:var(--secondary-text-color);">
                📦 ${n}${c?H` <span class="update-badge">⬆ ${l}</span>`:""}
              </span>
            `:H`<span class="repo-not-installed">${ue("notInstalled")}</span>`}
          </div>
        </div>
      </div>
    `}_renderListItem(e,t){const o=e.full_name||e.repository,a=e.manifest_name||e.name||o,r=t.find(([e,t])=>t===o),i=!!r,s=i?r[0]:null,n=e.installed_version||"",l=e.latest_version||"",c=e.has_update,d=e.stargazers_count||0,p=e.description||"";return H`
      <div class="repo-item" @click=${()=>this._openCardDetail(e)}>
        <div class="repo-info">
          <div class="repo-top">
            ...
            ${i?H`<span class="renamed-badge">🔄 ${s}</span>`:""}
          </div>
          <div class="repo-meta">
            <span class="repo-fullname">${o}</span>
            <span class="stars" style="color:#f9a825;">⭐ ${d>0?"number"==typeof d?d.toLocaleString():d:0}</span>
            ${e.installed?H`
              <span class="repo-version">📦 ${n}</span>
              ${c?H`<span class="update-badge">⬆ ${l}</span>`:""}
            `:H`<span class="repo-not-installed">${ue("notInstalled")}</span>`}
          </div>
        ...
        <div class="repo-actions">
          ...
          ${i?H`
            <button class="btn primary sm" @click=${e=>{e.stopPropagation(),this._replaceRenamedOneClick(s,o)}} ?disabled=${this._renamedRefreshing}>
              ${this._renamedRefreshing?"⏳":ue("replace")}
            </button>
    ...
    `}render(){const{archivedRepos:e,ignoredRepos:t,renamedEntries:o,customRepos:a,loading:r,_viewMode:i,_collapsed:s}=this;return H`
      ${r?H`
      ...
      <div class="section">
        <div class="section-header" @click=${()=>this._toggleSection("customRepos")}>
          ...
          <div class="view-toggle">
            <button class=${Wt({active:"card"===i})} @click=${e=>{e.stopPropagation(),this._setViewMode("card")}}>
              🃏 ${ue("catDashboard")||"卡片"}
            </button>
            <button class=${Wt({active:"list"===i})} @click=${e=>{e.stopPropagation(),this._setViewMode("list")}}>
              ☰ ${ue("list")||"列表"}
            </button>
          </div>
          <button class="btn sm" @click=${e=>{e.stopPropagation(),this._load()}} title="${ue("refresh")}" style="flex-shrink:0;">🔄</button>
        </div>
        <div class="section-content ${s.customRepos?"collapsed":""}">
            ...
            <div class="add-form">
              ...
              <div class="add-form-controls">
                ...
                <button class="btn primary sm" @click=${this._addCustomRepo} ?disabled=${this._addingCustom||!this._customRepoUrl.trim()}>
                  ${this._addingCustom?"⏳":"✓"} ${ue("add")}
                </button>
                <button class="btn sm" @click=${this._toggleAddCustom}>${ue("cancel")}</button>
              </div>
              ${this._customRepoUrl.trim()?H`
                <div class="add-preview">${this._parseRepoUrl(this._customRepoUrl)?H`📦 ${this._parseRepoUrl(this._customRepoUrl)}`:H`<span class="add-error">${ue("invalidRepoUrl")}</span>`}</div>
              `:""}
            </div>
          `:H`
          ...
          ${a.length>0?"card"===i?H`<div class="repo-cards">${a.map(e=>this._renderCard(e,o))}</div>`:H`<div class="repo-list">${a.map(e=>this._renderListItem(e,o))}</div>`:H`<div class="empty">${ue("noCustomRepos")}</div>`}
        </div>
      </div>
        ...
        <div class="section-content ${s.tools?"collapsed":""}">
          ...
          <div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:16px;">
            ...
            <button class="btn" @click=${this._checkDependencies} ?disabled=${this._depLoading}>
              ${this._depLoading?"⏳":"🔍"} ${this._depLoading?ue("checkingUpdates"):ue("checkDep")}
            </button>
          </div>
          ...
          ${this._depResults?H`
            <div class="dep-panel">
              <div class="dep-summary">
                <span class="title">${this._depResults.all_ok?"✅":"⚠️"} ${ue("totalPrefix")||"共"} ${this._depResults.total_checked} ${ue("totalRepos")}</span>
                ${this._depResults.all_ok?"":H`<span class="issues">${this._depResults.issues_count} ${ue("depMissing")||"缺失"}</span>`}
              </div>
              ${this._depResults.all_ok?H`<div class="dep-ok">✅ ${ue("depOk")}</div>`:H`
