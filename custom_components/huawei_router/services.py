        ...
        schema=vol.Schema(
            {
                ...
                vol.Required(_FIELD_PASSWORD): vol.All(vol.Coerce(str), vol.Length(min=8)),
            }
        ),
    ),
    # 端口映射管理服务
...
def _find_coordinator(hass: HomeAssistant, device_mac: MAC_ADDR) -> HuaweiDataUpdateCoordinator | None:
...
def _find_coordinator_serial(hass: HomeAssistant, serial_number: str) -> HuaweiDataUpdateCoordinator | None:
...
async def _async_add_to_whitelist(hass: HomeAssistant, service: ServiceCall):
    ...
    if not coordinator:
        raise HomeAssistantError(f"Can not find coordinator for mac address '{device_mac}'")
    _LOGGER.debug(
...
async def _async_setup_guest_network(hass: HomeAssistant, service: ServiceCall):
    ...
    if not coordinator:
        raise HomeAssistantError(f"Can not find coordinator with primary router's serial number '{serial_number}'")
    _LOGGER.debug(
    ...
    try:
        ...
        duration: HuaweiGuestNetworkDuration = _WIFI_DURATION_MAP[service.data[_FIELD_DURATION]]
        secured: bool = _WIFI_SECURITY_MAP[service.data[_FIELD_SECURITY]]
        password: str | None = service.data[_FIELD_PASSWORD]
        await coordinator.primary_router_api.set_guest_network_state(enabled, ssid, duration, secured, password)
    except Exception as ex:
        raise HomeAssistantError(str(ex))
...
async def _async_port_mapping_add(hass: HomeAssistant, service: ServiceCall):
    """Service to add port mapping."""
    internal_host = service.data["internal_host"]
    internal_port = service.data["internal_port"]
    external_port = service.data["external_port"]
    protocol = service.data["protocol"].upper()
    description = service.data.get("description", "")
    enabled = service.data.get("enabled", True)
    _LOGGER.debug(
        "Service '%s' called: internal_host=%s, internal_port=%s, external_port=%s, protocol=%s",
        service.service,
        internal_host,
        internal_port,
        external_port,
    ...
    try:
        from .client.classes import PortMapping, HuaweiPortMappingProtocol
        from .client.huaweiapi import HuaweiApi

        protocol_enum = HuaweiPortMappingProtocol.TCP if protocol == "TCP" else HuaweiPortMappingProtocol.UDP
        ...
        success = await coordinator.primary_router_api.add_port_mapping(new_mapping)
        if not success:
            raise HomeAssistantError("Failed to add port mapping")
        _LOGGER.info(
            "Port mapping added successfully: %s:%s -> %s:%s/%s",
            internal_host,
            internal_port,
            external_port,
            protocol,
...
async def _async_port_mapping_remove(hass: HomeAssistant, service: ServiceCall):
    """Service to remove port mapping."""
    external_port = service.data["external_port"]
    protocol = service.data["protocol"].upper()
    _LOGGER.debug(
        "Service '%s' called: external_port=%s, protocol=%s",
        service.service,
        external_port,
        protocol,
    )
    ...
    if not coordinator:
        raise HomeAssistantError("Can not find any Huawei router coordinator")
    try:
        from .client.classes import HuaweiPortMappingProtocol
        from .client.huaweiapi import HuaweiApi

        protocol_enum = HuaweiPortMappingProtocol.TCP if protocol == "TCP" else HuaweiPortMappingProtocol.UDP
        success = await coordinator.primary_router_api.remove_port_mapping(external_port, protocol_enum)
        if not success:
            raise HomeAssistantError("Failed to remove port mapping")
        _LOGGER.info("Port mapping removed successfully: %s/%s", external_port, protocol)
    except HomeAssistantError:
        raise
    except Exception as ex:
...
async def _async_port_mapping_list(hass: HomeAssistant, service: ServiceCall):
    ...
    # 返回端口映射列表
    mappings = []
    for mapping in coordinator.port_mappings:
        mappings.append(
            {
                "internal_host": mapping.internal_host,
                "internal_port": mapping.internal_port,
                "external_port": mapping.external_port,
                "protocol": mapping.protocol.value if hasattr(mapping.protocol, "value") else str(mapping.protocol),
