...
class HuaweiApi:
    ...
    async def get_router_info(self) -> HuaweiRouterInfo:
        ...
        data = await self._core_api.get(URL_DEVICE_INFO, check_authorized=HuaweiApi._router_data_check_authorized)
    ...
    async def get_wan_connection_info(self) -> HuaweiConnectionInfo:
        data = await self._core_api.get(URL_WANDETECT, check_authorized=HuaweiApi._wan_info_check_authorized)
    ...
    async def get_switch_state(self, switch: Switch) -> bool:
        ...
        elif switch == Switch.WIFI_80211R and self._features.is_available(Feature.WIFI_80211R):
        ...
        elif switch == Switch.WIFI_TWT and self._features.is_available(Feature.WIFI_TWT):
        ...
        elif switch == Switch.WLAN_FILTER and self._features.is_available(Feature.WLAN_FILTER):

            _, data = await self.get_wlan_filter_info()

            return data.enabled

        elif switch == Switch.GUEST_NETWORK and self._features.is_available(Feature.GUEST_NETWORK):

            data_2g, data_5g = await self.get_guest_network_info()

            return (data_2g is not None and data_2g.enabled) or (data_5g is not None and data_5g.enabled)
    ...
    async def get_known_devices(self) -> Iterable[HuaweiClientDevice]:
        ...
        return [HuaweiClientDevice(item) for item in await self._core_api.get(URL_HOST_INFO)]
    ...
    async def get_devices_topology(self) -> Iterable[HuaweiDeviceNode]:
        ...
        return [self._get_device(item) for item in await self._core_api.get(URL_DEVICE_TOPOLOGY)]
    ...
    async def apply_url_filter_info(self, url_filter_info: HuaweiUrlFilterInfo) -> None:
        ...
        data: dict[str, Any] = {
            "Devices": [{"MACAddress": device.mac_address} for device in url_filter_info.devices],
            "DeviceNames": [{"HostName": device.name} for device in url_filter_info.devices],
        ...
        if filter_action == FilterAction.REMOVE:
            ...
            if filter_mode == FilterMode.BLACKLIST:
                ...
                if blacklist_index is None:
                    ...
                    self._logger.debug("Can not find device '%s' to remove from blacklist", device_mac)
                ...
                if whitelist_index is None:
                    ...
                    self._logger.debug("Can not find device '%s' to remove from whitelist", device_mac)
    ...
    async def set_guest_network_state(
        ...
        config2g = self._to_guest_wifi_config(actual_2g, rsa_key, enabled, ssid, duration, secure, password)

        config5g = self._to_guest_wifi_config(actual_5g, rsa_key, enabled, ssid, duration, secure, password)
    ...
    async def get_port_mappings(
        ...
        return [HuaweiPortMappingItem.parse(item) for item in await self._core_api.get(URL_PORT_MAPPING)]
    ...
    async def set_port_mapping_state(self, port_mapping_id: str, enabled: bool) -> None:
        ...
        target = next((item for item in port_mappings if item.get("ID") == port_mapping_id))
        ...
        await self._core_api.post(URL_PORT_MAPPING, target, extra_data={"action": "update"})

    async def _set_guest_network_enabled(self, enabled: bool) -> None:

        actual_2g, actual_5g = await self.get_guest_network_info()

        actual_enabled = (actual_2g is not None and actual_2g.enabled) or (actual_5g is not None and actual_5g.enabled)
    ...
    async def set_time_control_item_state(self, time_control_item_id: str, enabled: bool) -> None:

        time_control_items = await self._core_api.get(URL_TIME_CONTROL)

        target = next((item for item in time_control_items if item.get("ID") == time_control_item_id))

        if not target:

            raise InvalidActionError(f"Unknown time control item: {time_control_item_id}")

        target["Enable"] = enabled

        await self._core_api.post(URL_TIME_CONTROL, target, extra_data={"action": "update"})
