...
class HuaweiPortMappingItem:
    ...
    def parse(cls, raw_data: dict[str, Any]) -> HuaweiPortMappingItem:
        ...
        return HuaweiPortMappingItem(id, name, enabled, ip_address, mac_address, host_name, application_id)
...
class HuaweiFilterInfo:
    ...
    def parse(cls, raw_data: dict[str, Any]) -> HuaweiFilterInfo:
        ...
        def get_item(raw_item: dict[str, Any]) -> HuaweiFilterItem:
            ...
            return HuaweiFilterItem(name=raw_item.get("HostName"), mac_address=raw_item.get("MACAddress"))
...
class HuaweiTimeControlItemDay:
    ...
    def __init__(self, day_of_week: DayOfWeek, is_enabled: bool, start: time, end: time):
        """Initialize."""
...
class HuaweiTimeControlItem:
    ...
    def __init__(self, data: Dict):
        ...
        for day_of_week in DayOfWeek:
            ...
            day: HuaweiTimeControlItemDay = HuaweiTimeControlItemDay(day_of_week, is_enabled, start, end)
    ...
    def name(self) -> str:
        ...
        device_names = [item.get("HostName", "device") for item in self._data.get("DeviceNames", [])]

        days = [active_day.day_of_week.value for active_day in filter(lambda x: x.is_enabled, self._days.values())]

        device_names_str = "for " + ", ".join(device_names) if len(device_names) > 0 else "for some devices"
