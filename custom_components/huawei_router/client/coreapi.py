...
class HuaweiCoreApi:
    def __init__(
        ...
        # Endpoints that returned 404 even after a re-authentication retry:
        # they are not supported by this firmware. Q6 returns 404 for missing
        # endpoints (an expired session does not), so retrying with a full
        # logout/re-login on every request is pure waste and kicks the admin
        # web session each time.
        self._missing_endpoints: set[str] = set()
    ...
    async def get(self, path: str, **kwargs: Any) -> Dict | List:
        ...
        if not check_authorized(response, result):
            if response.status == 404 and path in self._missing_endpoints:
                raise ApiCallError(
                    f"Endpoint {path} is not supported by the router",
            ...
            if not check_authorized(response, result):
                if response.status == 404:
                    self._logger.debug(
                        "Endpoint %s still returns 404 after re-authentication, "
                        "marking it as unsupported",
                        path,
                    )
                    self._missing_endpoints.add(path)
                raise ApiCallError(
