...
class HuaweiCoreApi:
    ...
    def _handle_public_key_dict(self, data: Dict | None) -> None:
        ...
        if data is not None and "rsan" in data and "rsae" in data and "rsapubkeysignature" in data:
    ...
    async def _post_raw(self, path: str, data: Dict, headers: dict = None) -> ClientResponse:
    ...
    async def authenticate(self) -> None:
        ...
        if not await self._init_csrf():
            ...
            raise AuthenticationError("Failed to get initial CSRF", AUTH_FAILURE_GENERAL)
        ...
        client_proof = get_client_proof(self._password, salt, iterations, first_nonce, server_nonce)
    ...
    async def _try_logout(self):
        ...
        try:
            ...
            response = await self._post_raw("api/system/user_logout", {"csrf": self._active_csrf})

            if response.status != 200:

                self._logger.debug("Can not perform logout: status is %s", response.status)
    ...
    async def get(self, path: str, **kwargs: Any) -> Dict | List:
        ...
        check_authorized: Callable[[ClientResponse, Dict], bool] = kwargs.get("check_authorized") or _check_authorized
