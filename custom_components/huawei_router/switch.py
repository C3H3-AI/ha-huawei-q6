...
async def _add_nfc_if_available(
    ...
    try:
        ...
        _LOGGER.debug("Could not check NFC feature at %s: %s", mac, err)
...
async def _add_access_switch_if_available(
    ...
    try:
        ...
        _LOGGER.debug("Could not check WLAN filter feature at %s: %s", mac, err)
...
def watch_for_wireless_devices(coordinator, config_entry, async_add_entities):
    ...
    known_access_switches: dict[MAC_ADDR, HuaweiSwitch] = {}
    device_watcher: ClientWirelessDevicesWatcher = ClientWirelessDevicesWatcher(coordinator)
    # 初始清理：移除离线设备的 WiFi 访问开关实体
    if skip_offline and is_wifi_switches_enabled:
        ...
        for mac, device in coordinator.connected_devices.items():
            ...
            for entity_entry in list(er.entities.values()):
                if (
                    ...
                    and _FUNCTION_ID_DEVICE_ACCESS in entity_entry.unique_id
    ...
    def on_wireless_device_added(device_mac: MAC_ADDR, device: ConnectedDevice) -> None:
        """When a new wireless device is detected."""
    ...
    @callback
    def on_wireless_device_removed(er, device_mac: MAC_ADDR, device: ConnectedDevice) -> None:
        """When a wireless device is removed (went offline)."""
        if skip_offline:
            entity = known_access_switches.pop(device_mac, None)
    ...
    def coordinator_updated() -> None:
        ...
        if is_wifi_switches_enabled:
            if coordinator._is_initial_update:
                ...
                return
            device_watcher.look_for_changes(
                on_wireless_device_added, on_wireless_device_removed if skip_offline else None
            )
...
def watch_for_url_filters(coordinator, config_entry, async_add_entities):
    ...
    @callback
    def on_filter_removed(er: EntityRegistry, filter_id: str, url_filter: UrlFilter) -> None:
        """When a known filter removed."""
        unique_id = generate_entity_unique_id(coordinator, _FUNCTION_ID_URL_FILTER, filter_id)
...
def watch_for_port_mappings(coordinator, config_entry, async_add_entities):
    ...
    known_port_mapping_switches: dict[str, HuaweiSwitch] = {}
    port_mappings_watcher: HuaweiPortMappingsWatcher = HuaweiPortMappingsWatcher(coordinator)
    ...
    @callback
    def on_port_mapping_removed(er: EntityRegistry, port_mapping_id: str, port_mapping: PortMapping) -> None:
        """When a known port mapping removed."""
        unique_id = generate_entity_unique_id(coordinator, _FUNCTION_ID_PORT_MAPPING, port_mapping_id)
        entity_id = er.async_get_entity_id(Platform.SWITCH, DOMAIN, unique_id)
        if entity_id:
            er.async_remove(entity_id)
    ...
    def coordinator_updated() -> None:
        ...
        if is_port_mapping_switches_enabled:
            port_mappings_watcher.look_for_changes(on_port_mapping_added, on_port_mapping_removed)
...
def watch_for_time_control_items(coordinator, config_entry, async_add_entities):
    ...
    time_controls_watcher: HuaweiTimeControlItemsWatcher = HuaweiTimeControlItemsWatcher(coordinator)

    @callback
    def on_time_control_added(time_control_id: str, time_control: HuaweiTimeControlItem) -> None:
    ...
    @callback
    def on_time_control_removed(er: EntityRegistry, time_control_id: str, time_control: HuaweiTimeControlItem) -> None:
        """When a known port mapping removed."""
        unique_id = generate_entity_unique_id(coordinator, _FUNCTION_ID_TIME_CONTROL, time_control_id)
        entity_id = er.async_get_entity_id(Platform.SWITCH, DOMAIN, unique_id)
        if entity_id:
            er.async_remove(entity_id)
    ...
    def coordinator_updated() -> None:
        ...
        if is_time_control_switches_enabled:
            time_controls_watcher.look_for_changes(on_time_control_added, on_time_control_removed)
...
class HuaweiSwitch(CoordinatorEntity[HuaweiDataUpdateCoordinator], SwitchEntity, ABC):
    ...
    async def async_added_to_hass(self) -> None:
        ...
        if self._device_mac:
            _LOGGER.debug("Switch %s (%s) added to hass", self._switch, self._device_mac)
        else:
    ...
    def is_on(self) -> bool | None:
        ...
        return self.coordinator.get_switch_state(self._switch, self._device_mac, self._switch_id)

    async def _go_to_state(self, state: bool):
        """Perform transition to the specified state."""
        await self.coordinator.set_switch_state(self._switch, state, self._device_mac, self._switch_id)
    ...
    def turn_on(self, **kwargs: Any) -> None:
        """turn_on."""
        return asyncio.run_coroutine_threadsafe(self.async_turn_on(**kwargs), self.hass.loop).result()

    def turn_off(self, **kwargs: Any) -> None:
        """turn_off."""
        return asyncio.run_coroutine_threadsafe(self.async_turn_off(**kwargs), self.hass.loop).result()
...
class HuaweiNfcSwitch(HuaweiSwitch):
    ...
    def __init__(
        ...
        self._attr_name = _FUNCTION_DISPLAYED_NAME_NFC
        self._attr_unique_id = generate_entity_unique_id(coordinator, _FUNCTION_ID_NFC, device.mac if device else None)
...
class HuaweiWifi80211RSwitch(HuaweiSwitch):
    ...
    def __init__(self, coordinator: HuaweiDataUpdateCoordinator) -> None:
        ...
        self._attr_name = _FUNCTION_DISPLAYED_NAME_WIFI_802_11_R
        self._attr_unique_id = generate_entity_unique_id(coordinator, _FUNCTION_ID_WIFI_802_11_R)
        self.entity_id = generate_entity_id(
            coordinator,
            ENTITY_DOMAIN,
...
class HuaweiWifiTWTSwitch(HuaweiSwitch):
    ...
    def __init__(self, coordinator: HuaweiDataUpdateCoordinator) -> None:
        ...
        self._attr_name = _FUNCTION_DISPLAYED_NAME_WIFI_TWT
        self._attr_unique_id = generate_entity_unique_id(coordinator, _FUNCTION_ID_WIFI_TWT)
        self.entity_id = generate_entity_id(
            coordinator,
            ENTITY_DOMAIN,
...
class HuaweiWlanFilterSwitch(HuaweiSwitch):
    ...
    def __init__(self, coordinator: HuaweiDataUpdateCoordinator) -> None:
        ...
        self._attr_name = _FUNCTION_DISPLAYED_NAME_WLAN_FILTER
        self._attr_unique_id = generate_entity_unique_id(coordinator, _FUNCTION_ID_WLAN_FILTER)
        self.entity_id = generate_entity_id(
            coordinator,
            ENTITY_DOMAIN,
...
class HuaweiGuestNetworkSwitch(HuaweiSwitch):
    ...
    def __init__(self, coordinator: HuaweiDataUpdateCoordinator) -> None:
        ...
        self._attr_name = _FUNCTION_DISPLAYED_NAME_GUEST_NETWORK
        self._attr_unique_id = generate_entity_unique_id(coordinator, _FUNCTION_ID_GUEST_NETWORK)
        self.entity_id = generate_entity_id(
            coordinator,
            ENTITY_DOMAIN,
...
class HuaweiDeviceAccessSwitch(HuaweiSwitch):
    ...
    def __init__(
        ...
        self._attr_name = f"{_FUNCTION_DISPLAYED_NAME_DEVICE_ACCESS}: {device.name}"
        self._attr_unique_id = generate_entity_unique_id(coordinator, _FUNCTION_ID_DEVICE_ACCESS, device.mac)
        self.entity_id = generate_entity_id(
            coordinator,
            ENTITY_DOMAIN,
...
class HuaweiUrlFilterSwitch(HuaweiSwitch):
    ...
    def __init__(
        ...
        super().__init__(coordinator, EmulatedSwitch.URL_FILTER, switch_id=filter_info.filter_id)
        self._attr_device_info = None
        self._attr_name = f"{_FUNCTION_DISPLAYED_NAME_URL_FILTER}: {filter_info.url}"
        self._attr_unique_id = generate_entity_unique_id(coordinator, _FUNCTION_ID_URL_FILTER, filter_info.filter_id)
        self.entity_id = generate_entity_id(
            coordinator,
            ENTITY_DOMAIN,
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        self._attr_name = f"{_FUNCTION_DISPLAYED_NAME_URL_FILTER}: {self._filter_info.url}"
...
class HuaweiPortMappingSwitch(HuaweiSwitch):
    ...
    def __init__(
        ...
        self._attr_extra_state_attributes = {}
        super().__init__(coordinator, EmulatedSwitch.PORT_MAPPING, switch_id=port_mapping.id)
        self._attr_device_info = coordinator.get_device_info()
        self._attr_name = f"{_FUNCTION_DISPLAYED_NAME_PORT_MAPPING}: {port_mapping.name}"
        self._attr_unique_id = generate_entity_unique_id(coordinator, _FUNCTION_ID_PORT_MAPPING, port_mapping.id)
        self.entity_id = generate_entity_id(
            coordinator,
            ENTITY_DOMAIN,
    ...
    def _handle_coordinator_update(self) -> None:
        """Handle updated data from the coordinator."""
        debug_data = (
            f"Id: {self._port_mapping.id}, name: {self._port_mapping.name}, enabled: {self._port_mapping.enabled}"
        )
        _LOGGER.debug("Switch %s: info is %s", self._switch_id, debug_data)
        self._attr_name = f"{_FUNCTION_DISPLAYED_NAME_PORT_MAPPING}: {self._port_mapping.name}"
...
class HuaweiTimeControlSwitch(HuaweiSwitch):
    ...
    def __init__(
        ...
        self._attr_extra_state_attributes = {}
        super().__init__(coordinator, EmulatedSwitch.TIME_CONTROL, switch_id=time_control.id)
        self._attr_device_info = None
        self._attr_name = f"{_FUNCTION_DISPLAYED_NAME_TIME_CONTROL}: {time_control.name}"
        self._attr_unique_id = generate_entity_unique_id(coordinator, _FUNCTION_ID_TIME_CONTROL, time_control.id)
        self.entity_id = generate_entity_id(
            coordinator,
            ENTITY_DOMAIN,
    ...
    def _handle_coordinator_update(self) -> None:
        """Handle updated data from the coordinator."""
        debug_data = (
            f"Id: {self._time_control.id}, name: {self._time_control.name}, enabled: {self._time_control.enabled}"
        )
        _LOGGER.debug("Switch %s: info is %s", self._switch_id, debug_data)
        self._attr_name = f"{_FUNCTION_DISPLAYED_NAME_TIME_CONTROL}: {self._time_control.name}"
        ...
        if self.coordinator.connected_devices:
            ...
            for device_mac in self._time_control.devices_mac:
                device: ConnectedDevice = self.coordinator.connected_devices.get(device_mac)
                if device:
