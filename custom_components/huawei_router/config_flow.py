            ...
            data_schema=vol.Schema(
                {
                    vol.Required(
                        ...
                        default=self._local_config_entry.options.get(
                            CONF_SCAN_INTERVAL,
                            self._local_config_entry.data.get(CONF_SCAN_INTERVAL, DEFAULT_SCAN_INTERVAL),
                        ),
                    ): int,
                }
            ),
            ...
            data_schema=vol.Schema(
                {
                    vol.Required(
                        OPT_WIFI_ACCESS_SWITCHES,
                        default=self.options.get(OPT_WIFI_ACCESS_SWITCHES, DEFAULT_WIFI_ACCESS_SWITCHES),
                    ): bool,
                    vol.Required(
                        OPT_ROUTER_CLIENTS_SENSORS,
                        default=self.options.get(OPT_ROUTER_CLIENTS_SENSORS, DEFAULT_ROUTER_CLIENTS_SENSORS),
                    ): bool,
                    vol.Required(
                        OPT_DEVICES_TAGS,
                        default=self.options.get(OPT_DEVICES_TAGS, DEFAULT_DEVICES_TAGS),
                    ): bool,
                    vol.Required(
                        OPT_DEVICE_TRACKER,
                        default=self.options.get(OPT_DEVICE_TRACKER, DEFAULT_DEVICE_TRACKER),
                    ): bool,
                    vol.Required(
                        OPT_DEVICE_TRACKER_ZONES,
                        default=self.options.get(OPT_DEVICE_TRACKER_ZONES, DEFAULT_DEVICE_TRACKER_ZONES),
                    ): bool,
                    vol.Required(
                        OPT_URL_FILTER_SWITCHES,
                        default=self.options.get(OPT_URL_FILTER_SWITCHES, DEFAULT_URL_FILTER_SWITCHES),
                    ): bool,
                    vol.Required(
                        OPT_EVENT_ENTITIES,
                        default=self.options.get(OPT_EVENT_ENTITIES, DEFAULT_EVENT_ENTITIES),
                    ): bool,
                    vol.Required(
                        OPT_PORT_MAPPING_SWITCHES,
                        default=self.options.get(OPT_PORT_MAPPING_SWITCHES, DEFAULT_PORT_MAPPING_SWITCHES),
                    ): bool,
                    vol.Required(
                        OPT_TIME_CONTROL_SWITCHES,
                        default=self.options.get(OPT_TIME_CONTROL_SWITCHES, DEFAULT_TIME_CONTROL_SWITCHES),
                    ): bool,
                    vol.Required(
                        OPT_SKIP_OFFLINE_DEVICES,
