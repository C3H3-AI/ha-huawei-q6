...
def get_coordinator(hass: HomeAssistant, config_entry: ConfigEntry) -> HuaweiDataUpdateCoordinator:
    result = hass.data.get(DOMAIN, {}).get(config_entry.entry_id, {}).get(DATA_KEY_COORDINATOR)
    if not result:
...
def pop_coordinator(hass: HomeAssistant, config_entry: ConfigEntry) -> HuaweiDataUpdateCoordinator | None:
...
def set_loaded_platforms(
    ...
    hass.data.setdefault(DOMAIN, {}).setdefault(config_entry.entry_id, {})[DATA_KEY_PLATFORMS] = platforms
...
def get_loaded_platforms(hass: HomeAssistant, config_entry: ConfigEntry) -> list[str]:
    result = hass.data.get(DOMAIN, {}).get(config_entry.entry_id, {}).get(DATA_KEY_PLATFORMS)
    if not result:
        raise ConfigurationError(f"Loaded plaforms not found at {config_entry.entry_id}")
    return result
