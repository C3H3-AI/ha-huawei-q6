    ...
    async def wrapper(*args, **kwargs):
        ...
        try:
            ...
            if not coordinator.is_unloaded:
                ...
                coordinator._logger.debug("Exception suppressed, coordinator is unloaded: %s", repr(ex))
...
class HuaweiDataUpdateCoordinator(DataUpdateCoordinator):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            update_interval=timedelta(seconds=self._integration_options.update_interval),
        )
    ...
    def zones_map(self) -> ZonesMap:
        ...
        if self._integration_options.device_tracker_zones:
            ...
            raise CoordinatorError("Devices tracker zones are disabled by integration options.")
    ...
    def diagnostics_info(self) -> dict:
        ...
        wifi_2_4 = sum(
            1 for d in self._connected_devices.values() if d.interface_type == HuaweiInterfaceType.INTERFACE_2_4GHZ
        )
        wifi_5 = sum(
            1 for d in self._connected_devices.values() if d.interface_type == HuaweiInterfaceType.INTERFACE_5GHZ
        )
        lan = sum(1 for d in self._connected_devices.values() if d.interface_type == HuaweiInterfaceType.INTERFACE_LAN)
    ...
    async def _update_time_control(self) -> None:
        ...
        time_control_items: dict[str, HuaweiTimeControlItem] = {
            item.id: item for item in await self.primary_router_api.get_time_control_items()
        }
    ...
    async def _update_port_mappings(self) -> None:
        ...
        new_port_mappings = {item.id: item for item in await self.primary_router_api.get_port_mappings()}
    ...
    async def _update_selects(self) -> None:
        ...
        if self._integration_options.device_tracker_zones:
            ...
            self._logger.debug("Zone (%s) state updated to %s", _PRIMARY_ROUTER_IDENTITY, state)
            for mac, api in self._apis.items():
                if mac == _PRIMARY_ROUTER_IDENTITY:
                    continue
                device = self._connected_devices.get(mac)
                self._logger.debug("Updating Zone select for %s", device.name if device else mac)
    ...
    async def update_router_nfc_switch(
        ...
        if await api.is_feature_available(Feature.NFC):
            ...
            self._logger.debug("Nfc switch (%s) state updated to %s", device.name, state)
    ...
    async def calculate_device_access_switch_states(
        ...
        if await self.primary_router_api.is_feature_available(Feature.WLAN_FILTER):
            for device in self.connected_devices.values():
                ...
                self._logger.debug("Device access switch (%s) state updated to %s", device.name, state)
    ...
    async def _update_connected_devices(self) -> None:
        ...
        if self._primary_router_mac is not None:
            primary_device = next(
                (item for item in devices_data if item.mac_address == self._primary_router_mac),
                None,
            )
        ...
        if not mesh_routers:
            _LOGGER.warning("No routers found from device topology. Trying fallback detection from HostInfo.")
            for device in devices_data:
                if device.is_router:
                    _LOGGER.info(
                    ...
                    mesh_routers.append(HuaweiDeviceNode(device.mac_address, "Device"))
        ...
        for device_data in devices_data:
            ...
            tags = self._tags_map.get_tags(mac) if self._integration_options.devices_tags else []
            ...
            if is_active:
                ...
                zone_name = (
                    next(
                        (zone.name for zone in self._zones if zone.entity_id == zone_id),
                        None,
                    )
                    if zone_id
                    else None
                ...
                if not self._is_initial_update:
                    if device.connected_via_id and device.connected_via_id != connected_via.get("id"):
    ...
    def _select_api(self, device_mac: MAC_ADDR | None) -> HuaweiApi:
        ...
        if not api:
            raise CoordinatorError(f"Can not find api for device '{device_mac or _PRIMARY_ROUTER_IDENTITY}'")
        return api

    async def is_feature_available(self, feature: Feature, device_mac: MAC_ADDR | None = None) -> bool:
    ...
    async def set_switch_state(
        ...
        if switch == EmulatedSwitch.DEVICE_ACCESS:
            ...
            await self.primary_router_api.apply_wlan_filter(filter_mode, FilterAction.ADD, device_mac)
        # URL filter switch processing
        elif switch == EmulatedSwitch.URL_FILTER:
            if not switch_id:
                raise CoordinatorError(f"Can not set value: switch_id is required for {EmulatedSwitch.URL_FILTER}")
        ...
        # Port mapping switch processing
            ...
            if not switch_id:
                raise CoordinatorError(f"Can not set value: switch_id is required for {EmulatedSwitch.PORT_MAPPING}")
            port_mapping = self._port_mappings.get(switch_id)
            if port_mapping.enabled == state:
                return
            port_mapping.set_enabled(state)
            await self.primary_router_api.set_port_mapping_state(port_mapping.id, port_mapping.enabled)
        # Time control switch processing
        elif switch == EmulatedSwitch.TIME_CONTROL:
            if not switch_id:
                raise CoordinatorError(f"Can not set value: switch_id is required for {EmulatedSwitch.TIME_CONTROL}")
            time_control = self._time_control_items.get(switch_id)
            if time_control.enabled == state:
                return
            time_control.set_enabled(state)
            await self.primary_router_api.set_time_control_item_state(time_control.id, time_control.enabled)
        # other switches
        else:
    ...
    def get_select_state(self, select: Select, device_mac: MAC_ADDR | None = None) -> str | None:
    ...
    async def set_select_state(self, select: Select, state: str, device_mac: MAC_ADDR | None = None) -> None:
        """Set state of the specified select."""
        api = self._select_api(device_mac)
        # WLAN Filter Mode select
        if select == Select.WLAN_FILTER_MODE and await api.is_feature_available(Feature.WLAN_FILTER):
        ...
        # Router's zone select
            ...
            await self._zones_map.set_zone_id(device_mac or _PRIMARY_ROUTER_IDENTITY, state)
        # Unknown select
        else:
    ...
    async def execute_action(self, action: Action, device_mac: MAC_ADDR | None = None) -> None:
    ...
    # ---------------------------
    @callback
    def async_subscribe_event(self, event_types: list[str], handle_callback: CALLBACK_TYPE) -> Callable[[], None]:
