...
_FUNCTION_DISPLAYED_NAME_WLAN_FILTER_MODE: Final = "WiFi访问控制模式"
...
_FUNCTION_DISPLAYED_NAME_ZONE: Final = "区域"
...
class HuaweiSelect(CoordinatorEntity[HuaweiDataUpdateCoordinator], SelectEntity, ABC):
    ...
    def select_option(self, option: str) -> None:
        """Handle option changed."""
        return asyncio.run_coroutine_threadsafe(self.async_select_option(option), self.hass.loop).result()
...
class HuaweiMappedSelect(HuaweiSelect, ABC):
    ...
    def __init__(
        ...
        super().__init__(coordinator, select, device_mac, list(values_to_displayed.values()))
        self._values_to_displayed = dict(values_to_displayed)
        self._displayed_to_values = {value: key for key, value in values_to_displayed.items()}
    ...
    def _process_updated_values(self, values_to_displayed: dict[str, str]):
        ...
        new_displayed_to_values = {value: key for key, value in values_to_displayed.items()}
...
class HuaweiWlanFilterModeSelect(HuaweiSelect):
    ...
    def __init__(
        ...
        """Initialize."""
        super().__init__(coordinator, Select.WLAN_FILTER_MODE, None, _OPTIONS_WLAN_FILTER_MODE)
        self._attr_name = _FUNCTION_DISPLAYED_NAME_WLAN_FILTER_MODE
        self._attr_unique_id = generate_entity_unique_id(coordinator, _FUNCTION_UID_WLAN_FILTER_MODE, None)
...
class HuaweiRouterZoneSelect(HuaweiMappedSelect):
    ...
    def __init__(self, coordinator: HuaweiDataUpdateCoordinator, device: ConnectedDevice | None) -> None:
        """Initialize."""
        super().__init__(coordinator, Select.ROUTER_ZONE, device.mac if device else None)
        self._attr_name = _FUNCTION_DISPLAYED_NAME_ZONE
        self._attr_unique_id = generate_entity_unique_id(coordinator, _FUNCTION_UID_ZONE, self._device_mac)
        self.entity_id = generate_entity_id(
            coordinator,
            ENTITY_DOMAIN,
